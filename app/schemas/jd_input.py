...
class JobDescriptionInput(BaseModel):
    ...
    designation: str = Field(
        min_length=1,
        description="Designation for the job role"
    )

    domain: str = Field(
        description="Department or domain (e.g., IT, Finance)"
    )
    ...
    max_experience: float = Field(
        ge=0,
        description="Maximum years of experience allowed"
    )
    ...
    number_of_positions: int = Field(
        ge=1,
        description="Number of openings available"
    )

    location: str = Field(
        description="Job location (e.g., Remote, Mumbai)"
    )
    ...
    big4_experience: Optional[Literal[1, 2]] = Field(
        default=None,
        description="Big 4 experience: 1=Yes, 2=No"
    )

    travel_required: Optional[Literal[1, 2, 3]] = Field(
        default=None,
        description="Travel requirement: 1=No, 2=Occasional, 3=Frequent"
    )

    store_db: bool = Field(
        description="Whether to store data in the database"
    )
